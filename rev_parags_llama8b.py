...
def generate_revision(messages,parag):
    ...
    outputs = model.generate(
        input_ids,
        max_new_tokens=len(parag)+50,
        eos_token_id=terminators,
        do_sample=True,
        temperature=0.6,
        top_p=0.9,
    )
    reponse = outputs[0][input_ids.shape[-1]:]
    reponse=tokenizer.decode(reponse, skip_special_tokens=True).strip()
...
def revision_from_labels_approche1_separate(parag):
    ...
    for intention in parag[2]:
        ...
        {"role": "user", "content": fill_pattern_user(edit_intent,before_text)}]
...
path_sortie="predictions_llama_rev_parag/"
...
for idx in range(len(liste_data_inputs)):
        file_sortie=open(path_sortie+"predict_llama"+filename, 'a') 
    #try:
